...
<?php
    ...
    $select = "SELECT * FROM `datphong` WHERE `tenkhach` LIKE '%$searchValue%'and `trangthaithanhtoan`='chuathanhtoan'ORDER BY `id` DESC LIMIT $offset, $sobg";
    ...
    }$tong_bg = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM `datphong` WHERE `tenkhach` LIKE '%$searchValue%'and `trangthaithanhtoan`='chuathanhtoan'"));
    $soluongtrang = ceil($tong_bg / $sobg);//hàm làm tròn
    mysqli_close($conn);
?>
<div class="timkiem">
    <form method="post" style="display: flex; width: 70%; margin: 0 auto;justify-content: space-between" action="indexadmin.php?danhmuc=danhsachphongchuathanhtoan">
...
<table align="center">
    <tbody>
        <tr>
            <td colspan="10" align="center"><h3>Danh sách phòng chưa thanh toán</h3></td>
        </tr>
        <tr align="center">
            <th>STT</th>
            <th>ID phòng</th>
        ...
        <tr>
            ...
            <td><?php echo $trangthaithanhtoan[$i]; ?></td>
